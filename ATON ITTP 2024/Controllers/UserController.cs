    ...
    public class UserController : Controller
    {
        ...
        public async Task<ActionResult> ReadActiveUsers(string login, string password)
        {
            try
            {
                ...
                if (!user.Admin)
                {
                    throw new Exception("Только администраторы могут выполнять этот запрос");
                }
        ...
        /// <param name="isAdmin">Является ли новый пользователь администратором</param>
        /// <returns>Новый созданный объект User</returns>
        /// <response code="200">Успешное выполнение</response>
        /// <response code="400">Ошибка API</response>
        [HttpPost("Create")]
        [ProducesResponseType(typeof(User), (int)HttpStatusCode.OK)]
        [ProducesResponseType(typeof(ErrorMessage), (int)HttpStatusCode.BadRequest)]
        public async Task<ActionResult> Create(string login, string password, string name, int gender, DateTime? birthday, bool isAdmin)
        {
            try
            {
                return Ok(await _service.CreateAsync(login, password, name, gender, birthday, isAdmin));
            }
            catch (Exception ex)
            {
                return BadRequest(new ErrorMessage() { Message = ex.Message, ExceptionType = ex.GetType().Name });
